...
auto CAntiMultipleFarm::__GetPlayerStartupDropState(std::string sMAIf) -> bool
{
	...
	if ((int)it->second.size() > MULTIPLE_FARM_MAX_ACCOUNT) return true;
...
{
	// Fix: when a player teleports we do not remove from the map
...
auto CAntiMultipleFarm::__ReloadBlockDropStates(std::string sMAIf) -> void
{
	...
	if ((int)it->second.size() <= MULTIPLE_FARM_MAX_ACCOUNT)
	{
...
{
	// Fix cross-fire between server stages (auth server has no drop concept)
	...
	TSendAntiFarmInfo dataPacket(GC::ANTI_FARM, 0, AF_SH_SENDING_DATA);
	...
	for (; it_send < iterate_map->second.end(); it_send++) {
		...
		if (buf.size()) {
			...
			CHARACTER::SafeSendPacketTo(pkTarget, buf.read_peek(), buf.size());
		}
		else CHARACTER::SafeSendPacketTo(pkTarget, &dataPacket, sizeof(dataPacket));
	}
}
...
auto CAntiMultipleFarm::PrintPlayerDropState(std::string sMAIf, LPCHARACTER ch) -> void
{
	...
	for (auto &sR : iterate_map->second)
		ch->ChatPacket(CHAT_TYPE_INFO, "PID: %d | State: %d (1=Blocked / 0=Unblocked)", sR.dwPID, (int)sR.bDropState);
	ch->ChatPacket(CHAT_TYPE_INFO, "------------------------------------");
}
